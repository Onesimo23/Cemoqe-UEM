...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
  ...
  const menuItems = [
    ...
    { icon: <Users className="w-5 h-5" />, label: 'Gestão de Utilizadores', href: '/admin/usuarios' },
    { icon: <UserCheck className="w-5 h-5" />, label: 'Gestão de Tutores', href: '/admin/tutores' },
    { icon: <Layers className="w-5 h-5" />, label: 'Gestão de Conteúdos', href: '/admin/conteudos' },
    { icon: <FileText className="w-5 h-5" />, label: 'Moderação de Cursos', href: '/admin/moderacao' },
    { icon: <Key className="w-5 h-5" />, label: 'Gestão de Permissões', href: '/admin/permissoes' },
    { icon: <BarChart3 className="w-5 h-5" />, label: 'Relatório e Analytics', href: '/admin/analytics' },
    { icon: <Settings className="w-5 h-5" />, label: 'Configurações do Sistema', href: '/admin/configuracoes' },
  ];
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans">
        ...
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold">
              ...
              <span className="ml-1 text-[10px] bg-brand-accent text-brand-dark px-1.5 py-0.5 rounded font-black uppercase">Admin</span>
            </span>
          </Link>
        ...
        <div className="p-6 border-b border-white/10 bg-white/5">
          <div className="flex items-center gap-3">
            { (profile?.avatar_url || user?.photoURL) ? (
              <img src={profile?.avatar_url || user?.photoURL || ''} className="w-10 h-10 rounded-full border-2 border-brand-accent/30 object-cover" alt="Admin" />
            ) : (
            ...
            <div>
              <p className="font-semibold text-sm">{profile?.full_name || 'Super Admin'}</p>
              <p className="text-[10px] text-brand-accent font-black uppercase tracking-widest">Acesso de Nível 1</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="hidden md:flex bg-white border-b border-slate-200 py-4 px-8 items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-6">
             ...
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Servidores Operacionais</span>
             </div>
          </div>
          <div className="flex items-center gap-4">
            ...
            { (profile?.avatar_url || user?.photoURL) ? (
              <img src={profile?.avatar_url || user?.photoURL || ''} className="w-8 h-8 rounded-full border-2 border-slate-100 object-cover" alt="Admin" />
            ) : (
              <div className="w-8 h-8 rounded-full bg-brand-green text-white font-bold grid place-items-center border-2 border-slate-100">AD</div>
            )}
          </div>
        </header>
        ...
        <header className="bg-slate-900 text-white border-b border-white/5 py-4 px-6 md:hidden flex items-center justify-between sticky top-0 z-30">
          ...
          <span className="font-bold">EduPrime <span className="text-brand-accent">Admin</span></span>
          <div className="w-8 h-8 bg-brand-green rounded-full flex items-center justify-center font-bold text-xs">AD</div>
        </header>

...
const ForumPage: React.FC = () => {
  ...
  const [newTopic, setNewTopic] = useState({ title: '', category: 'Técnico', content: '' });
  const [enrolledIds, setEnrolledIds] = useState<string[]>([]);

  useEffect(() => {
    if (!user?.uid) { setTopics([]); setEnrolledIds([]); return; }
    ...
    const recompute = (docs: Array<{ id: string; data: any }>) => {
      ...
      const mapped: Topic[] = docs.map(({ id, data }) => {
        const created: Date | null = data?.createdAt?.toDate ? data.createdAt.toDate() : (data?.updatedAt?.toDate ? data.updatedAt.toDate() : null);
        return {
          id,
          title: data?.title || 'Tópico',
          author: data?.user_name || data?.author || 'Estudante',
          avatar: data?.avatar || 'https://i.pravatar.cc/150?img=11',
          category: (data?.category || 'Geral') as any,
          snippet: (data?.content || data?.body || '').toString().slice(0, 120) + '...',
    ...
    const subscribeQuestions = (courseIds: string[]) => {
      ...
      chunks.forEach(ids => {
        ...
        const u = onSnapshot(qQ, (snap) => {
          ...
          for (let i = collected.length - 1; i >= 0; i--) {
            const cid = collected[i].data?.course_id || collected[i].data?.courseId;
      ...
      const qG = query(collection(db, 'questions'), where('course_id', '==', null as any));
    ...
    return () => { if (enrollUnsubA) enrollUnsubA(); if (enrollUnsubB) enrollUnsubB(); qUnsubs.forEach(u => u()); };
  }, [user?.uid]);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Troque conhecimento com outros estudantes e instrutores.</p>
          </div>
          <button 
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                >
                  <span className="text-gray-600 group-hover:text-brand-green">Todas</span>
                  <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{filteredTopics.length}</span>
                </button>
                {CATEGORIES.map(cat => (
                  <button 
                    key={cat}
                    ...
                    className="w-full text-left px-3 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors flex items-center justify-between group"
                  >
                    <span className="text-gray-600 group-hover:text-brand-green">{cat}</span>
                    <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
                      {filteredTopics.filter(t => t.category === (cat as any)).length}
                    </span>
                  </button>
                ))}
              </div>
            ...
            <div className="bg-gradient-to-br from-brand-dark to-brand-green p-6 rounded-xl text-white shadow-xl">
               ...
               <p className="text-sm text-brand-light/80 mb-4 leading-relaxed">Compartilhe um bug que você resolveu hoje e como fez isso!</p>
               <button className="text-xs font-bold bg-white text-brand-green px-4 py-2 rounded-lg hover:bg-brand-accent hover:text-brand-dark transition-colors">Participar agora</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {filteredTopics.length > 0 ? (
              filteredTopics.map(topic => (
                <div key={topic.id} className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm hover:border-brand-green/30 transition-all group cursor-pointer">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                       <span className={`text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider ${
                         topic.category === 'Técnico' ? 'bg-blue-50 text-blue-600' :
                         topic.category === 'Projetos' ? 'bg-purple-50 text-purple-600' :
                         topic.category === 'Carreira' ? 'bg-orange-50 text-orange-600' :
                  ...
                  <p className="text-sm text-gray-500 mb-5 line-clamp-2">{topic.snippet}</p>

                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <img src={topic.avatar} className="w-8 h-8 rounded-full border border-gray-100" alt={topic.author} />
                      <span className="text-sm font-semibold text-gray-700">{topic.author}</span>
                    </div>
                    
                    <div className="flex items-center gap-4 text-gray-400">
                      <div className="flex items-center gap-1.5 hover:text-brand-green transition-colors">
                        <MessageCircle className="w-4 h-4" />
                        <span className="text-xs font-bold">{topic.replies}</span>
                      </div>
              ...
              <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-gray-200">
                ...
                <h3 className="text-lg font-bold text-gray-900">Nenhum tópico encontrado</h3>
                <p className="text-gray-500 text-sm">Ajuste seu filtro ou seja o primeiro a postar!</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
               <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                  <h3 className="font-bold text-gray-900 text-lg">Criar Nova Discussão</h3>

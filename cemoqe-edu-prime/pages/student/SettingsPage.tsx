...
const SettingsPage: React.FC = () => {
  ...
  const handleChangeFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!finalUrl) throw new Error('Nenhum provedor disponível para upload da imagem.');
      ...
      alert('Não foi possível atualizar a foto de perfil. Verifique sua conexão ou as configurações do bucket (Firebase/Supabase).');
    } finally {
      setUploading(false);
          ...
          <div className="flex-1 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
              ...
              <form onSubmit={handleSave} className="space-y-6 animate-in fade-in duration-300">
                <div className="flex items-center gap-6 mb-8">
                  <div className="relative group">
                    <img 
                      src={previewUrl || profile?.avatar_url || user?.photoURL || "https://i.pravatar.cc/150?img=11"} 
                      className="w-24 h-24 rounded-full border-4 border-gray-50 shadow-md object-cover" 
                      alt="Avatar"
                    />
                    {uploading && (
                      <div className="absolute inset-0 bg-black/30 rounded-full grid place-items-center text-white text-xs font-bold">Carregando...</div>
                    )}
                    <button type="button" onClick={handlePickImage} className="absolute bottom-0 right-0 bg-brand-green text-white p-2 rounded-full shadow-lg hover:scale-110 transition-transform">
                ...
                <div className="flex items-center justify-between pt-6 border-t border-gray-100">
                  {isSaved && <span className="flex items-center gap-2 text-green-600 text-sm font-bold"><CheckCircle className="w-4 h-4" /> Alterações salvas!</span>}
                  <button type="submit" className="flex items-center gap-2 bg-brand-green text-white px-6 py-2.5 rounded-lg font-bold hover:bg-brand-dark transition-all ml-auto shadow-lg shadow-green-900/20">
              ...
              <form onSubmit={handleSave} className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="password" placeholder="••••••••" className="w-full px-4 py-2.5 bg-[#333333] border-transparent rounded-xl focus:ring-4 focus:ring-brand-green/20 text-white outline-none" />
                  </div>
                  <div>
                ...
                <div className="pt-6 border-t border-gray-100">
                  <button type="submit" className="flex items-center gap-2 bg-brand-dark text-white px-6 py-2.5 rounded-lg font-bold hover:bg-black transition-all">
                    Atualizar Senha
                  </button>
                </div>
              </form>
            ...
            {activeTab === 'notifications' && (
               <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div className="space-y-4">
                    <NotificationToggle label="E-mails sobre novos cursos" description="Receba atualizações quando lançarmos novos conteúdos." defaultChecked />
                    <NotificationToggle label="Notificações de fórum" description="Avisar quando alguém responder seus tópicos." defaultChecked />
                    <NotificationToggle label="Lembretes de estudo" description="Avisos semanais para manter sua rotina de estudos." />
                  </div>
               </div>
            )}
          </div>
...
const TabItem = ({ active, onClick, icon, label }: any) => (
  <button 
    ...
    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${
      active ? 'bg-brand-green text-white shadow-lg shadow-green-900/10' : 'text-gray-500 hover:bg-white hover:text-brand-dark'
    }`}
  >
    {icon} {label}
  </button>
...
const NotificationToggle = ({ label, description, defaultChecked = false }: any) => (

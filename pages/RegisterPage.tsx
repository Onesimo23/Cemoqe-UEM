...
import { Eye, EyeOff, Lock, Mail, User, ArrowRight, CheckCircle, Loader2, AlertCircle } from 'lucide-react';
...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Como o padrão é estudante, redirecionamos para a landing page
      navigate('/');
    } catch (err: any) {
      console.error(err);
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row border border-gray-100">
        ...
        <div className="md:w-1/2 bg-brand-dark relative hidden md:flex flex-col justify-between p-12 text-white">
          <div className="relative z-10">
            ...
            <p className="text-brand-light/80 leading-relaxed">
              Junte-se à maior comunidade de aprendizado online e transforme o seu futuro hoje.
            </p>
          </div>
          
          <div className="relative z-10 space-y-5">
            <div className="flex items-center gap-3">
              <div className="bg-brand-green/20 p-2 rounded-lg"><CheckCircle className="w-5 h-5 text-brand-accent" /></div>
              <span className="text-sm font-medium">Acesso a cursos exclusivos</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="bg-brand-green/20 p-2 rounded-lg"><CheckCircle className="w-5 h-5 text-brand-accent" /></div>
              <span className="text-sm font-medium">Certificados verificados</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="bg-brand-green/20 p-2 rounded-lg"><CheckCircle className="w-5 h-5 text-brand-accent" /></div>
              <span className="text-sm font-medium">Suporte com tutores reais</span>
            </div>
          </div>
        ...
        <div className="md:w-1/2 p-8 md:p-12">
          <div className="text-center md:text-left mb-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-2">Cadastre-se grátis</h3>
            <p className="text-gray-500 text-sm">
              Já tem uma conta?{' '}
              <Link to="/login" className="text-brand-green font-bold hover:underline">Faça login</Link>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-start">
              ...
              <label htmlFor="terms" className="ml-2 text-sm text-gray-500">
                Li e aceito os <Link to="/termos" className="text-brand-green font-medium">Termos de Uso</Link> e a <Link to="/privacidade" className="text-brand-green font-medium">Política de Privacidade</Link>.
              </label>
            </div>

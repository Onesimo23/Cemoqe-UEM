...
import { doc, onSnapshot, collection, getDocs, query, where, limit as qbLimit, addDoc, serverTimestamp } from 'firebase/firestore';
  ...
  const normalizedModules = useMemo(() => {
    ...
    return modulesArr.map((m: any, mi: number) => {
      ...
      const lessonsArr = toArray(lessonsRaw).map((l: any, li: number) => ({
        ...
        type: l?.type ?? l?.kind ?? l?.lessonType ?? ((l?.content ?? l?.url ?? l?.videoUrl) ? 'video' : (l?.texto ? 'text' : (l?.file ? 'document' : 'text'))),
        content: l?.content ?? l?.url ?? l?.videoUrl ?? l?.texto ?? l?.file ?? '',
  ...
  const ytEmbed = (url?: string) => {
    ...
    try {
      ...
      if (u.hostname.includes('youtube.com')) {
        ...
        if (vid) return `https://www.youtube.com/embed/${vid}?start=0&end=30&rel=0&modestbranding=1&playsinline=1`;
      }
    } catch {}
    return null;
  };
  ...
  if (!loading && !course) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        ...
        <Link to="/cursos" className="text-brand-green font-semibold hover:underline">Voltar para Cursos</Link>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 relative z-10">
            ...
            <div className="flex items-center gap-2 text-sm text-brand-light/70 mb-6 font-medium">
              ...
              <Link to="/cursos" className="hover:text-white transition-colors">{course?.category || 'Geral'}</Link>
              <span className="text-xs">›</span>
              <span className="text-white truncate max-w-[200px]">{course?.title || 'Curso'}</span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm mb-8">
              <div className="flex items-center gap-1">
                <span className="font-bold text-brand-accent">{course?.rating ?? 0}</span>
                <div className="flex text-brand-accent">
                  {[1, 2, 3, 4, 5].map(i => <Star key={i} className="w-3.5 h-3.5 fill-current" />)}
                </div>
                <span className="text-brand-light/70 ml-1">({(course?.reviewCount ?? 0).toLocaleString()} avaliações)</span>
              </div>
              <span className="hidden sm:inline text-brand-light/40">|</span>
              <div className="flex items-center gap-1 text-brand-light/90">
                <span className="text-brand-light/70">Instrutor:</span>
                <span className="underline decoration-brand-accent/50 underline-offset-4 font-semibold">{course?.instructor || 'Instrutor'}</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <section className="bg-gray-50 border border-gray-200 p-6 md:p-8 rounded-2xl">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {Array.isArray(course?.learningOutcomes) && course.learningOutcomes.length > 0 ? (
                  course.learningOutcomes.slice(0, 8).map((t: string, i: number) => (
            ...
            <section>
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-200">
                {Array.isArray(displayModules) && displayModules.length > 0 ? (
                  ...
                  <div className="p-6 text-sm text-gray-500">Currículo ainda não publicado.</div>
                )}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="prose text-gray-600 max-w-none leading-relaxed">
                {course?.fullDescription ? (
                  <p className="mb-4 whitespace-pre-line">{course.fullDescription}</p>
                ) : (
                  <>
                    <p className="mb-4">
                ...
                <h3 className="text-lg font-bold text-gray-800 mt-6 mb-2">Para quem é este curso:</h3>
                <ul className="list-disc pl-5 space-y-1">
                  <li>Iniciantes que desejam entrar na área de {course?.category || 'Geral'}.</li>
                  <li>Profissionais que buscam atualização e novas técnicas.</li>
                  <li>Estudantes universitários querendo complementar o currículo.</li>
                </ul>
              </div>
            </section>
            ...
            <section className="bg-white border-t border-gray-100 pt-8">
              ...
              <div className="flex gap-6 items-start">
                ...
                />
                <div>
                  <h3 className="text-xl font-bold text-gray-900 mb-1">{course?.instructor || 'Instrutor'}</h3>
                  <p className="text-brand-green font-medium text-sm mb-3">Senior {course?.category || 'Geral'} Specialist</p>
                ...
                <div className="relative aspect-video group cursor-pointer bg-gray-900">
                  {(!isEnrolled && firstVideo) ? (
                    (() => {
                      ...
                      return (
                        <TimedHtml5Preview src={firstVideo?.content} />
                      );
                    })()
                  ) : (
                    <img src={course?.imageUrl} alt="Preview" className="w-full h-full object-cover opacity-90 group-hover:opacity-75 transition-opacity" />
                  )}
                  <div className="absolute bottom-4 left-0 w-full text-center">
                    <span className="text-white font-bold text-sm drop-shadow-md">Pré-visualizar curso</span>
                  </div>
                </div>

                <div className="p-6">
                  <div className="flex items-end gap-3 mb-6">
                    <span className="text-3xl font-bold text-gray-900">{(course?.priceType === 'free') ? 'Gratuito' : `${course?.currency || 'MZM'} ${course?.price || '0,00'}`}</span>
                  </div>

                  {isEnrolled ? (
                    <Link to={`/aluno/sala-de-aula/${course?.id || id}`} className="w-full inline-flex items-center justify-center bg-brand-green hover:bg-brand-dark text-white font-bold py-4 rounded-lg shadow-lg shadow-brand-green/20 transition-all mb-3 text-lg">
                      Continuar Curso
                    </Link>
                  ) : (
                    course?.priceType === 'free' ? (
                      <button
                        ...
                        disabled={enrolling}
                        className="w-full bg-brand-green hover:bg-brand-dark disabled:opacity-70 text-white font-bold py-4 rounded-lg shadow-lg shadow-brand-green/20 transition-all mb-3 text-lg"
                      >
                        {enrolling ? 'Inscrevendo...' : 'Inscrever-se Gratuitamente'}
                      </button>
                    ) : (
                      <>
                        <button className="w-full bg-brand-green hover:bg-brand-dark text-white font-bold py-4 rounded-lg shadow-lg shadow-brand-green/20 transition-all mb-3 text-lg">
                          Comprar Agora
                        </button>
                        <button className="w-full bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold py-3 rounded-lg transition-colors mb-6">
                  ...
                  <p className="text-xs text-center text-gray-500 mb-6">Garantia de reembolso de 30 dias</p>

                  <div className="space-y-4 pt-6 border-t border-gray-100">
                    <h4 className="font-bold text-gray-900 text-sm">Este curso inclui:</h4>
                    <FeatureRow icon={<Infinity className="w-4 h-4" />} text="Acesso vitalício total" />
                    <FeatureRow icon={<Smartphone className="w-4 h-4" />} text="Acesso no celular e TV" />
                    <FeatureRow icon={<FileText className="w-4 h-4" />} text="Exercícios e projetos práticos" />
                    <FeatureRow icon={<Download className="w-4 h-4" />} text="Recursos para download" />
...
const AccordionItem = ({ title, details, isOpen = false, lessons = [] as any[] }: { title: string, details: string, isOpen?: boolean, lessons?: any[] }) => {
          ...
          <div className="space-y-3">
            {Array.isArray(lessons) && lessons.length > 0 ? (
              lessons.map((lsn: any, idx: number) => {
                ...
                const Icon = t === 'text' ? FileText : t === 'document' ? File : PlayCircle;
                const dur = lsn?.duration || '';
                return (
                  <div key={lsn?.id || idx} className="flex items-center gap-3 text-sm text-gray-600">
                    <Icon className="w-4 h-4 text-gray-400" />
                    <span>{lsn?.title || `Aula ${idx + 1}`}</span>
                    {dur ? <span className="ml-auto text-xs text-gray-400">{dur}</span> : null}
                  </div>
                );
              })
            ) : (

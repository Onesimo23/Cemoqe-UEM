...
const INITIAL_PENDING: PendingCourse[] = [
  { 
    ...
    duration: '12h',
    date: 'Há 45 min',
    thumbnail: 'https://images.unsplash.com/photo-1516116216624-53e697fedbea?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80'
  },
  { 
    id: 'p2', 
    ...
    duration: '18h',
    date: 'Há 3 horas',
    thumbnail: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80'
  },
  { 
    id: 'p3', 
...
const CourseModerationPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedPending) {
      ...
      localStorage.setItem('eduprime_pending_moderation', JSON.stringify(INITIAL_PENDING));
    }
  }, []);

  const updatePendingStorage = (newList: PendingCourse[]) => {
    setPendingList(newList);
    localStorage.setItem('eduprime_pending_moderation', JSON.stringify(newList));
  };
  ...
  const handleApprove = (id: string) => {
    ...
    const catalog: Course[] = savedCourses ? JSON.parse(savedCourses) : MOCK_COURSES;
  ...
  const handleReject = (e: React.FormEvent) => {
    ...
    console.log(`Feedback enviado para ${rejectedCourse?.instructor}: ${rejectionReason}`);
    
    // Save to a mock "notifications" system for the instructor
    const notifications = JSON.parse(localStorage.getItem('eduprime_notifications') || '[]');
    notifications.push({
      id: Date.now(),
    ...
    localStorage.setItem('eduprime_notifications', JSON.stringify(notifications));

    const updatedPending = pendingList.filter(item => item.id !== rejectingId);
    updatePendingStorage(updatedPending);
    ...
    <AdminLayout>
      <div className="max-w-6xl mx-auto space-y-8 pb-20">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Moderação de Conteúdos</h1>
            <p className="text-slate-500 mt-1">Revise e autorize novos cursos submetidos por instrutores.</p>
          </div>
        ...
        {showSuccess && (
          <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl flex items-center gap-3 text-emerald-700 animate-in fade-in slide-in-from-top-2 duration-300">
             ...
             <p className="text-xs font-bold uppercase tracking-tight">{showSuccess}</p>
          </div>
        )}
        ...
        <div className="bg-brand-light/30 border border-brand-green/10 rounded-[32px] p-6 flex gap-4">
           ...
           <div>
              <h4 className="font-bold text-brand-dark text-sm">Diretrizes da Plataforma</h4>
              <p className="text-xs text-brand-dark/60 mt-1 leading-relaxed font-medium">
                 Verifique se o áudio está nítido, se as descrições seguem as regras de SEO e se os materiais complementares estão acessíveis. 
                 Aprovações levam o curso diretamente à vitrine pública e para a Gestão de Conteúdos.
              </p>
           </div>
        </div>
        ...
        <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden">
           <div className="p-6 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fila de Prioridade</span>
              </div>
           </div>
           ...
           <div className="divide-y divide-slate-100">
              {pendingList.length > 0 ? (
                ...
                <div className="py-24 flex flex-col items-center justify-center text-slate-300">
                   ...
                   <h3 className="font-bold text-slate-400 uppercase text-xs tracking-widest">Tudo limpo por aqui!</h3>
                   <p className="text-xs text-slate-400 mt-1">Nenhum curso aguardando moderação no momento.</p>
                </div>
              )}
           </div>
        </div>
        ...
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-[380px] rounded-[32px] p-8 shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-100">
              <div className="flex items-center justify-between mb-6">
                 <h3 className="font-black text-slate-900 text-sm uppercase tracking-widest">Reprovar Conteúdo</h3>
                 <button onClick={() => setRejectingId(null)} className="text-slate-300 hover:text-slate-900 transition-all">
                   <X size={20} />
                 </button>
              </div>
  ...
  <div className="p-6 md:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 hover:bg-slate-50/30 transition-colors group animate-in slide-in-from-left-4 duration-500">
     <div className="flex gap-6">
        <div className="w-32 h-20 bg-slate-900 rounded-2xl overflow-hidden relative group cursor-pointer shrink-0 border border-slate-100 shadow-sm">
           ...
           <img src={course.thumbnail} className="w-full h-full object-cover opacity-60 group-hover:opacity-40 transition-opacity" alt="" />
        ...
        <div>
           <div className="flex items-center gap-2 mb-1.5">
              <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-brand-light text-brand-green rounded-md tracking-tighter">Novo Conteúdo</span>
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{course.date}</span>
           </div>
           <h4 className="font-black text-slate-800 text-lg leading-tight group-hover:text-brand-green transition-colors">{course.title}</h4>
           <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div> {course.category}</div>
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div> {course.instructor}</div>

...
import { 
  BarChart3, TrendingUp, Users, DollarSign, Download, Calendar, ArrowUpRight,
  BookOpen, Eye, CheckCircle, AlertCircle, Award, Zap, Activity, Target,
  Filter, FileText, PieChart, LineChart as LineChartIcon, Clock
} from 'lucide-react';
...
interface AnalyticsData {
  ...
  coursePerformance: Array<{ title: string; rating: number; enrollments: number; completion: number }>;
}
...
const AdminAnalyticsPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'visao-geral' | 'usuarios' | 'cursos' | 'receita' | 'performance' | 'pivot'>('visao-geral');
  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      try {
        ...
        const publishedCourses = coursesSnap.docs.filter(doc => doc.data().isActive !== false).length;
        console.log('📚 Total de Cursos:', publishedCourses, 'de', coursesSnap.size);
        ...
        const enrollmentsSnap = allEnrollmentsSnap.docs.filter(doc => doc.data().certificatePaid === true);
        let totalRevenue = 0;
        enrollmentsSnap.forEach(doc => {
          const data = doc.data();
          const price = typeof data.certificatePrice === 'string' 
            ? parseFloat(data.certificatePrice.replace(/\./g, '').replace(',', '.'))
        ...
        const certificationRate = totalEnrollments > 0 
        ...
        const completionRate = totalEnrollments > 0
          ...
          : 0;
        console.log('✅ Cursos Completos:', completedEnrollments, 'de', totalEnrollments, '=', completionRate, '%');
        console.log('✅ Taxa de Conclusão:', completionRate, '% -', completedEnrollments, 'de', totalEnrollments);
        ...
        coursesSnap.forEach(doc => {
          ...
          courseCategories.set(category, (courseCategories.get(category) || 0) + 1);
        });
        ...
        allEnrollmentsSnap.docs.forEach(enrollment => {
          const courseId = enrollment.data().course_id || enrollment.data().courseId;
          const course = topCourses.find(c => c.id === courseId);
          if (course) {
            course.enrollments++;
            if (enrollment.data().certificatePaid) {
              const price = typeof enrollment.data().certificatePrice === 'string'
                ? parseFloat(enrollment.data().certificatePrice.replace(/\./g, '').replace(',', '.'))
        ...
        const coursePerformance = topCoursesSorted.map(course => {
          ...
          const completedForCourse = enrollmentsForCourse.filter(
            e => (e.data().progress || 0) >= 100 || e.data().completed === true
          ).length;
          const courseCompletion = enrollmentsForCourse.length > 0
            ? Math.round((completedForCourse / enrollmentsForCourse.length) * 100)
            : 0;
        ...
        setData({
          ...
          activeUsers,
          courseCategories: Array.from(courseCategories.entries()).map(([name, count]) => ({ name, count })),
          topCourses: topCoursesSorted.map(c => ({ title: c.title, enrollments: c.enrollments, revenue: c.revenue })),
          revenueBySource,
          userGrowth,
          coursePerformance
        });
  ...
  const loadPivotData = async () => {
    try {
      ...
      enrollmentsSnap.docs.forEach(enrollDoc => {
        ...
        const studentDoc = studentsSnap.docs.find(doc => doc.id === studentUid);
        const studentData = studentDoc?.data();
        
        if (studentData) {
          const studentName = studentData.full_name || studentData.name || 'Sem nome';
      ...
      console.log('✅ Dados Pivot carregados:', pivotArray.length, 'estudantes');
    } catch (error) {
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-2">Visão completa de todos os dados do sistema</p>
          </div>
                  ...
                  {data?.userGrowth.map((item, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center group">
                      <div 
                        ...
                        style={{ height: `${(item.count / Math.max(...(data?.userGrowth || []).map(x => x.count))) * 100}%` }}
                      >
                      ...
                      <span className="text-xs font-bold text-slate-500 mt-2">{item.month}</span>
                    </div>
                  ))}
                </div>
              </div>
                    ...
                    <div key={i} className="space-y-1">
                      ...
                      <div className="w-full h-2 bg-white/20 rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${(cat.count / Math.max(...(data?.courseCategories || []).map(x => x.count))) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  ))}
        ...
        {activeTab === 'usuarios' && (
          <div className="space-y-6">
            <div className="grid md:grid-cols-3 gap-6">
              <MetricBox label="Total de Utilizadores" value={data?.totalStudents.toLocaleString() || '0'} icon={<Users size={24} />} color="bg-blue-50 text-blue-600" />
              <MetricBox label="Utilizadores Ativos" value={data?.activeUsers.toLocaleString() || '0'} icon={<Activity size={24} />} color="bg-green-50 text-brand-green" />
              <MetricBox label="Tutores Verificados" value={data?.activeTutors.toLocaleString() || '0'} icon={<CheckCircle size={24} />} color="bg-purple-50 text-purple-600" />
            </div>
            
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
              <h3 className="font-bold text-slate-800 mb-6">Distribuição de Utilizadores</h3>
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm font-semibold mb-2">
                    <span>Estudantes</span>
                    <span>{Math.round((data?.totalStudents || 0) / (data?.totalStudents || 1) * 100)}%</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-sm font-semibold mb-2">
                    <span>Tutores</span>
                    <span>{Math.round((data?.activeTutors || 0) / (data?.totalStudents || 1) * 100)}%</span>
                  </div>
                  <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-purple-500" style={{ width: `${Math.round((data?.activeTutors || 0) / (data?.totalStudents || 1) * 100)}%` }}></div>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'cursos' && (
          <div className="space-y-6">
            <div className="grid md:grid-cols-3 gap-6">
              <MetricBox label="Total de Cursos" value={data?.publishedCourses.toLocaleString() || '0'} icon={<BookOpen size={24} />} color="bg-orange-50 text-orange-600" />
              <MetricBox label="Total de Inscrições" value={data?.totalEnrollments.toLocaleString() || '0'} icon={<Users size={24} />} color="bg-blue-50 text-blue-600" />
              <MetricBox label="Rating Médio" value={`${data?.avgRating.toFixed(1) || '0.0'}/5`} icon={<Award size={24} />} color="bg-yellow-50 text-yellow-600" />
            </div>
                ...
                {data?.topCourses.map((course, i) => (
                  <div key={i} className="border-b border-slate-100 last:border-0 pb-4 last:pb-0">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-semibold text-slate-900">{i + 1}. {course.title}</h4>
                        <p className="text-sm text-slate-500 mt-1">{course.enrollments} inscrições • MZM {course.revenue.toLocaleString()}</p>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-brand-green">{Math.round((course.enrollments / (data?.totalEnrollments || 1)) * 100)}%</div>
        ...
        {activeTab === 'receita' && (
          <div className="space-y-6">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-800 mb-6">Distribuição de Receita</h3>
                <div className="space-y-4">
                  {data?.revenueBySource.map((item, i) => (
                    <div key={i}>
                      <div className="flex justify-between text-sm font-semibold mb-2">
                        <span>{item.source}</span>
                        <span className="text-brand-green">MZM {item.amount.toLocaleString()}</span>
                      </div>
                      <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                        <div 
                          className={i === 0 ? "h-full bg-brand-green" : "h-full bg-brand-accent"}
                          style={{ width: `${(item.amount / (data?.totalRevenue || 1)) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-800 mb-6">Resumo Financeiro</h3>
                <div className="space-y-4">
                  <div className="p-4 bg-slate-50 rounded-2xl">
                    <p className="text-xs text-slate-500 mb-1">Receita Bruta</p>
                    <p className="text-2xl font-bold text-slate-900">MZM {(data?.totalRevenue || 0).toLocaleString()}</p>
                  </div>
                  <div className="p-4 bg-green-50 rounded-2xl border border-green-200">
                    <p className="text-xs text-brand-green mb-1">Receita para Plataforma (15%)</p>
                    <p className="text-2xl font-bold text-brand-green">MZM {Math.round((data?.totalRevenue || 0) * 0.15).toLocaleString()}</p>
                  </div>
                  <div className="p-4 bg-blue-50 rounded-2xl border border-blue-200">
                    <p className="text-xs text-blue-600 mb-1">Pagamento para Tutores (85%)</p>
                    <p className="text-2xl font-bold text-blue-600">MZM {Math.round((data?.totalRevenue || 0) * 0.85).toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'performance' && (
          <div className="space-y-6">
            <div className="grid md:grid-cols-3 gap-6">
              <MetricBox label="Taxa de Conclusão" value={`${data?.completionRate || 0}%`} icon={<CheckCircle size={24} />} color="bg-green-50 text-brand-green" />
              <MetricBox label="Rating Médio" value={`${data?.avgRating.toFixed(1) || '0.0'}/5`} icon={<Star size={24} />} color="bg-yellow-50 text-yellow-600" />
              <MetricBox label="Engajamento" value={`${Math.round((data?.completionRate || 0) * 1.2)}%`} icon={<Zap size={24} />} color="bg-purple-50 text-purple-600" />
            </div>

            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
              <h3 className="font-bold text-slate-800 mb-6">Performance dos Cursos</h3>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="text-left py-3 px-4 font-bold text-slate-700 text-sm">Curso</th>
                      <th className="text-center py-3 px-4 font-bold text-slate-700 text-sm">Rating</th>
                      <th className="text-center py-3 px-4 font-bold text-slate-700 text-sm">Inscrições</th>
                      <th className="text-center py-3 px-4 font-bold text-slate-700 text-sm">Conclusão</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data?.coursePerformance.map((course, i) => (
                      <tr key={i} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                        ...
                        <td className="py-3 px-4 text-center font-semibold text-slate-900">{course.enrollments}</td>
                        <td className="py-3 px-4 text-center">
                          <span className={`inline-flex px-3 py-1 rounded-full text-sm font-bold ${
                            course.completion >= 75 ? 'bg-green-100 text-brand-green' : 'bg-orange-100 text-orange-600'
                          }`}>
                            {course.completion}%
                          </span>
                        </td>
        ...
        {activeTab === 'pivot' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-slate-900">Inscrições por Estudante</h2>
                <p className="text-slate-500 text-sm">Visualize todos os cursos de cada estudante</p>
              </div>
              <div className="flex gap-2">
                <input
                  type="text"
            ...
            {pivotLoading ? (
              <div className="flex items-center justify-center py-12">
                <div className="text-center">
                  ...
                  <p className="mt-3 text-slate-600">Carregando dados pivot...</p>
                </div>
              </div>
            ) : (
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                        <th className="text-left py-4 px-6 font-bold text-slate-700">Estudante</th>
                        <th className="text-left py-4 px-6 font-bold text-slate-700">Email</th>
                        <th className="text-center py-4 px-6 font-bold text-slate-700">Total de Inscrições</th>
                        <th className="text-left py-4 px-6 font-bold text-slate-700">Cursos Inscritos</th>
                      </tr>
                    </thead>
                    <tbody>
                      {pivotData
                        .filter(student =>
                          searchPivot === '' ||
                          student.studentName.toLowerCase().includes(searchPivot.toLowerCase()) ||
                          student.studentEmail.toLowerCase().includes(searchPivot.toLowerCase())
                        )
                        .map((student, idx) => (
                                    ...
                                    <div className="flex-1">
                                      ...
                                      <div className="flex items-center gap-2 mt-1">
                                        <div className="flex-1 h-1 bg-slate-200 rounded-full overflow-hidden">
                                          <div
                                            className={`h-full ${
                                              course.progress >= 75 ? 'bg-brand-green' : course.progress >= 50 ? 'bg-yellow-500' : 'bg-orange-500'
                                            }`}
                                            style={{ width: `${course.progress}%` }}
                                          ></div>
                                        </div>
                                        <span className="text-xs font-semibold text-slate-600 min-w-[45px]">{course.progress}%</span>
                                      </div>
                                    </div>
                ...
                {pivotData.filter(s =>
                  searchPivot === '' ||
                  s.studentName.toLowerCase().includes(searchPivot.toLowerCase()) ||
                  s.studentEmail.toLowerCase().includes(searchPivot.toLowerCase())
                ).length === 0 && (
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <MetricBox
                ...
                value={pivotData.reduce((sum, s) => sum + s.enrollmentCount, 0).toString()}
              ...
              <MetricBox
                ...
                value={(pivotData.length > 0
                  ? (pivotData.reduce((sum, s) => sum + s.enrollmentCount, 0) / pivotData.length).toFixed(1)
                  : '0').toString()}
...
const MetricBox = ({ label, value, icon, color }: any) => (
  <div className={`${color} p-6 rounded-2xl border border-current border-opacity-10 shadow-sm`}>

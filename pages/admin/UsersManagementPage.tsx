...
const UsersManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setUsers(usersList.sort((a, b) => a.full_name.localeCompare(b.full_name)));
      setLoading(false);
    }, (err) => {
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        ...
        const { user: newUser } = await createUserWithEmailAndPassword(tempAuth, formData.email, formData.password);
      ...
      if (tempAppToClean) {
        try { await deleteApp(tempAppToClean); } catch (e) {}
      }
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => {
      ...
      const roleMap: any = { 'student': 'Alunos', 'instructor': 'Tutores', 'admin': 'Admins' };
      const matchesRole = roleFilter === 'Todos' || roleMap[user.role] === roleFilter;
    ...
    <AdminLayout>
      <div className="space-y-8 pb-12 relative">
        ...
        {successMessage && (
          <div className="fixed top-24 right-8 z-[200] animate-in slide-in-from-right duration-500">
             <div className="bg-emerald-600 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border border-emerald-400/20">
                ...
                <span className="font-black uppercase text-[10px] tracking-widest">{successMessage}</span>
             </div>
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight">Gestão de Utilizadores</h1>
            <p className="text-slate-500 font-medium">Controle total da base de dados ({users.length} registos).</p>
          </div>
          <button 
        ...
        <div className="bg-white rounded-[40px] border border-slate-100 shadow-2xl overflow-hidden min-h-[500px]">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-48 gap-4">
               ...
               <p className="text-slate-400 font-black uppercase text-[10px] tracking-widest">A carregar registos...</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50/80 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Cargo / Status</th>
                    <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Supabase Sync</th>
                    ...
                    <tr key={user.id} className="hover:bg-slate-50/40 transition-colors group">
                      <td className="px-8 py-7">
                        <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg border shadow-inner ${user.status === 'Ativo' ? 'bg-brand-light/40 text-brand-green border-brand-green/5' : 'bg-slate-100 text-slate-300'}`}>
                            {user.full_name?.charAt(0) || '?'}
                          </div>
                          <div>
                            <p className="font-black text-slate-900 leading-tight">{user.full_name}</p>
                            ...
                            >
                               <Fingerprint size={10} /> {user.uid.substring(0, 12)}...
                               {copiedId === user.uid ? <CheckCircle2 size={10} className="text-brand-green" /> : <Copy size={10} />}
                            </button>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-7">
                         <div className="flex flex-col items-center gap-1.5">
                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-[10px] font-black uppercase border ${
                              user.role === 'admin' ? 'bg-brand-accent/10 text-brand-dark border-brand-accent/20' :
                              user.role === 'instructor' ? 'bg-brand-light text-brand-green border-brand-green/20' : 
                              'bg-blue-50 text-blue-600 border-blue-100'
                            }`}>
                              <Shield size={10} /> {user.role === 'admin' ? 'Admin' : user.role === 'instructor' ? 'Tutor' : 'Aluno'}
                            </span>
                            <span className={`text-[9px] font-black uppercase ${user.status === 'Ativo' ? 'text-emerald-600' : 'text-red-600'}`}>
                              {user.status}
                            </span>
                         </div>
                      </td>
                      <td className="px-6 py-7 text-center">
                         <div className="flex items-center justify-center gap-2">
                           <Database size={14} className={isSupabaseConfigured ? "text-emerald-500" : "text-slate-200"} />
                           <span className="text-[10px] font-black text-slate-400 uppercase">Online</span>
                         </div>
                      </td>
                      <td className="px-8 py-7 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => handleOpenModal(user)} className="p-3 text-slate-400 hover:text-brand-green bg-white border border-slate-100 rounded-xl transition-all shadow-sm active:scale-90">
                            <Edit size={18} />
                          </button>
                          <button onClick={() => setDeleteConfirmId(user.id)} className="p-3 text-slate-400 hover:text-red-600 bg-white border border-slate-100 rounded-xl shadow-sm transition-all active:scale-90">
                            <Trash2 size={18} />
                          </button>
                        </div>
                      </td>
                    ...
                    <tr>
                      <td colSpan={4} className="py-40 text-center">
                         <div className="flex flex-col items-center gap-4">
                            <div className="bg-slate-50 p-8 rounded-full"><Search size={42} className="text-slate-200" /></div>
                            <p className="text-slate-400 font-black uppercase text-xs tracking-widest">Vazio</p>
                         </div>
                      </td>
                    </tr>
                  )}
            ...
            <div className="bg-white w-full max-w-lg rounded-[48px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100">
               <div className="p-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Segurança Firebase & Supabase</p>
                    </div>
                  </div>
                  <button onClick={() => setIsUserModalOpen(false)} className="p-3 text-slate-300 hover:text-slate-900 transition-all hover:rotate-90">
                    <X size={28} />
                  </button>
               </div>
               ...
               <form onSubmit={handleSaveUser} className="p-10 space-y-6">
                  ...
                  {!editingUser && (
                    <div className="space-y-2 animate-in slide-in-from-top-2">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <button 
                          type="button"
                          onClick={() => setFormData({...formData, password: generateDefaultPassword(formData.full_name)})}
                  ...
                  <div className="flex gap-4 pt-6">
                    ...
                    >
                      {actionLoading ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : (editingUser ? 'Atualizar' : 'Salvar')}
                    </button>
                  </div>
               </form>
            </div>
        ...
        {deleteConfirmId && (
          <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl animate-in fade-in">
             <div className="bg-white w-full max-w-sm rounded-[48px] p-12 text-center shadow-2xl border border-slate-100">
                ...
                <h3 className="text-xl font-black text-slate-900 mb-2 uppercase tracking-tighter">Eliminar Registo?</h3>
                <p className="text-sm text-slate-500 mb-10 leading-relaxed font-medium">A remoção deste utilizador é definitiva no Firebase e Supabase.</p>

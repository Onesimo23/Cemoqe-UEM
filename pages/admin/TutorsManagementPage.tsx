...
const AdminTutorsPage: React.FC = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, 'profiles'), where('role', '==', 'instructor'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const list = snapshot.docs.map(d => {
        ...
        const specialty = (data.specialty || data.area || data.category || '—') as string;
        const status = ((data.status || 'Ativo') as 'Ativo' | 'Suspenso');
        const joinedAt = data.createdAt?.toDate?.() ? data.createdAt.toDate().toLocaleDateString() : '—';
        return {
          id: d.id,
          name: fullName,
          email,
  ...
  const filteredTutors = useMemo(() => {
    return tutors.filter(t => {
      ...
      const matchesSpecialty = specialtyFilter === 'Todas' || t.specialty === specialtyFilter;
  ...
  const handleToggleTutorStatus = async (id: string) => {
    ...
    try {
      ...
      if (isSupabaseConfigured) {
        await supabase.from('profiles').upsert({ id, status: newStatus, last_sync: new Date().toISOString() });
      }
    } catch (e) {
    ...
    <AdminLayout>
      <div className="space-y-8 pb-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Gestão de Tutores</h1>
            <p className="text-slate-500 mt-1">Controle de especialistas e novas candidaturas de ensino.</p>
          </div>
        </div>

        {/* Global Stats */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
           <TutorQuickStat label="Tutores Ativos" value={tutors.filter(t => t.status === 'Ativo').length.toString()} color="text-brand-green" />
           <TutorQuickStat label="Petições Pendentes" value={applications.length.toString()} color="text-brand-accent" />
           <TutorQuickStat label="Nota Média da Rede" value="4.85" color="text-emerald-500" />
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Especialista</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Alunos</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Rating</th>
             ...
             <div className="w-full max-w-lg bg-white h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
                <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                   <div className="flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl transition-colors ${selectedTutor.status === 'Suspenso' ? 'bg-slate-100 text-slate-400' : 'bg-brand-light text-brand-green'}`}>
                         {selectedTutor.name.charAt(0)}
                      </div>
                      <div>
                        <h3 className="font-black text-slate-900 text-lg uppercase tracking-tight">{selectedTutor.name}</h3>
                        <p className="text-xs text-brand-green font-bold uppercase tracking-widest">{selectedTutor.specialty}</p>
                      </div>
                   </div>
                   <button 
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8">
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                         ...
                         <p className="text-lg font-bold text-slate-900">{selectedTutor.joinedAt}</p>
                      </div>
                      <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Status Atual</p>
                         <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-xs font-bold ${selectedTutor.status === 'Ativo' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                            {selectedTutor.status === 'Ativo' ? <Check size={14} strokeWidth={3} /> : <Ban size={14} />} 
                   ...
                   <div className="space-y-4">
                      <h4 className="font-black text-slate-900 uppercase text-xs tracking-widest border-l-4 border-brand-green pl-3">Métricas de Performance</h4>
                      <div className="grid grid-cols-3 gap-3">
                         <MetricSmall label="Cursos" value={selectedTutor.courses.toString()} icon={<BookOpen size={12} />} />
                         <MetricSmall label="Alunos" value={selectedTutor.students} icon={<Users size={12} />} />
                         <MetricSmall label="Rating" value={selectedTutor.rating.toString()} icon={<Star size={12} />} />
                      </div>
                   </div>

                   {/* Contact */}
                   <div className="space-y-4">
                      <h4 className="font-black text-slate-900 uppercase text-xs tracking-widest border-l-4 border-brand-green pl-3">Contacto Directo</h4>
                      <div className="p-4 bg-white border border-slate-200 rounded-2xl flex items-center justify-between group cursor-pointer hover:border-brand-green transition-all">
                         <div className="flex items-center gap-3">
                            <Mail className="text-slate-400 group-hover:text-brand-green" size={18} />
                            <span className="text-sm font-bold text-slate-700">{selectedTutor.email}</span>
                         </div>
                         <ExternalLink size={14} className="text-slate-300 group-hover:text-brand-green" />
                      </div>
                   </div>

                   {/* History Timeline */}
                   <div className="space-y-4 pt-4">
                      <h4 className="font-black text-slate-900 uppercase text-xs tracking-widest border-l-4 border-brand-green pl-3">Actividade Recente</h4>
                      <div className="space-y-4 pl-3">
                         <ActivityItem text="Lançou novo módulo em 'UI Design Pro'" date="Há 3 dias" />
                         <ActivityItem text="Aprovou 15 certificados de conclusão" date="Há 5 dias" />
                         <ActivityItem text="Recebeu avaliação 5.0 (Ricardo M.)" date="Há 1 semana" highlight />
                      </div>
                   </div>
                </div>
                ...
                <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex gap-3 mt-auto">
                   ...
                   >
                     {selectedTutor.status === 'Ativo' ? 'Suspender Acesso' : 'Ativar Acesso'}
                   </button>
                   <button 
                    onClick={() => navigate('/admin/conteudos', { state: { instructorFilter: selectedTutor.name } })}
...
const TutorQuickStat = ({ label, value, color }: any) => (
  <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-all group">
     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 group-hover:text-brand-green transition-colors">{label}</p>
     ...
     <div className="flex items-center gap-4">
        ...
        <div>
           ...
           <div className="flex items-center gap-2 mt-0.5">
              <p className="text-[10px] text-brand-green font-black uppercase tracking-tight">{app.specialty}</p>
              <span className="text-slate-300 text-xs">•</span>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{app.exp} exp</p>
           </div>
        </div>
     </div>
     <div className="flex items-center gap-6">
        <span className="text-[10px] text-slate-400 font-black uppercase hidden sm:block">{app.date}</span>
...
const TutorTableRow: React.FC<{ 
  ...
  <tr className={`hover:bg-slate-50/50 transition-colors group ${tutor.status === 'Suspenso' ? 'opacity-70 bg-slate-50/30' : ''}`}>
    <td className="px-8 py-6">
       <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm group-hover:bg-brand-green group-hover:text-white transition-all ${tutor.status === 'Suspenso' ? 'bg-slate-200 text-slate-400' : 'bg-brand-light/50 text-brand-green'}`}>
             {tutor.name.charAt(0)}
          </div>
          <div>
            <p className={`font-bold text-sm transition-colors ${tutor.status === 'Suspenso' ? 'text-slate-400 italic' : 'text-slate-800 group-hover:text-brand-green'}`}>{tutor.name}</p>
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-tighter">{tutor.specialty}</p>
          </div>
       </div>
    </td>
    <td className="px-8 py-6">
       <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-[10px] font-black uppercase ${tutor.status === 'Ativo' ? 'text-emerald-600 bg-emerald-50' : 'text-red-600 bg-red-50'}`}>
          <span className={`w-1.5 h-1.5 rounded-full ${tutor.status === 'Ativo' ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
          {tutor.status}
       </span>
    </td>
    <td className="px-8 py-6 text-center">
       <div className="flex flex-col items-center">
          <span className="text-sm font-black text-slate-700">{tutor.students}</span>
          <span className="text-[9px] text-slate-400 font-bold uppercase tracking-tight">Activos</span>
       </div>
    </td>
    <td className="px-8 py-6">
...
const ActivityItem = ({ text, date, highlight }: any) => (
  <div className="relative pl-6 before:absolute before:left-0 before:top-2 before:w-2 before:h-2 before:bg-brand-green/30 before:rounded-full">
     <p className={`text-xs font-bold leading-tight ${highlight ? 'text-brand-green' : 'text-slate-700'}`}>{text}</p>
     <p className="text-[9px] text-slate-400 font-bold uppercase mt-0.5">{date}</p>
  </div>
);

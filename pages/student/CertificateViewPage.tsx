...
import { ChevronLeft, Download, Printer, GraduationCap, Award, Loader2 } from 'lucide-react';
...
const CertificateViewPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'courses', id), (snap) => {
      ...
      const total = Array.isArray((data as any)?.modules) 
        ? (data as any).modules.reduce((acc: number, m: any) => acc + (Array.isArray(m?.lessons) ? m.lessons.length : 0), 0)
  ...
  const handleDownloadPng = async () => {
    ...
    try {
      ...
      link.download = `Certificado-EduPrime-${course?.title.replace(/\s+/g, '-')}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
      console.error('Erro ao gerar imagem:', err);
      alert('Ocorreu um erro ao baixar o certificado. Tente imprimir como PDF.');
    } finally {
      setIsDownloading(false);
    }
  };

  if (!course) return <div className="p-20 text-center font-bold">Carregando certificado...</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-8 flex flex-col items-center overflow-x-hidden">
      ...
      <div className="w-full max-w-5xl flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 print:hidden">
        <Link to="/aluno/certificados" className="flex items-center gap-2 text-gray-600 hover:text-brand-dark font-semibold transition-colors">
        ...
        >
          ...
          <div className="m-[16px] flex-1 flex flex-col relative overflow-hidden bg-white">
            ...
            <div className="flex-1 flex flex-col items-center text-center p-16 relative z-0">
              ...
              <h1 className="text-5xl font-serif text-brand-dark mb-4 tracking-tighter uppercase font-medium">Certificado de Conclusão</h1>
              <div className="w-32 h-1 bg-brand-accent mb-10"></div>

              <p className="text-xl text-gray-400 font-medium mb-4 italic font-sans">Certificamos que</p>
              <h2 className="text-6xl font-bold text-gray-900 mb-8 font-serif leading-none">{profile?.full_name || 'Estudante'}</h2>
              ...
              <p className="text-xl text-gray-500 max-w-2xl leading-relaxed mb-10 font-sans">
                ...
                <strong className="text-brand-green text-3xl block mt-4 font-bold tracking-tight">{course.title}</strong>
                <span className="block mt-4 text-lg">com carga horária total de {course?.duration || `${totalLessons} aulas`}.</span>
              </p>

              {/* Bottom Signatures Area */}
              <div className="mt-auto w-full grid grid-cols-3 gap-12 items-end px-10">
                <div className="flex flex-col items-center">
                  <div className="w-full border-b-2 border-gray-200 pb-2 mb-2 italic text-gray-800 font-serif text-lg">Julia Santos</div>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[2px]">Coordenadora Acadêmica</p>
                </div>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[1px]">Selo de Autenticidade</p>
                </div>

                <div className="flex flex-col items-center">
                  <div className="w-full border-b-2 border-gray-200 pb-2 mb-2 font-mono text-gray-800 text-lg">24/10/2024</div>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[2px]">Data de Emissão</p>
                </div>
              </div>

              <div className="mt-10 text-[9px] text-gray-300 font-mono tracking-widest uppercase">
                ID de Autenticação: EP-{Math.random().toString(36).substr(2, 9).toUpperCase()} • Verifique em eduprime.com/verify
              </div>
            </div>
          </div>
        </div>

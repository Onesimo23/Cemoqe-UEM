...
import { Award, Download, Lock, CheckCircle, Clock, FileText } from 'lucide-react';
...
const CertificatesPage: React.FC = () => {
  ...
  const completedCount = useMemo(() => items.filter(c => c.progress === 100).length, [items]);
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-10">
          ...
          <p className="text-gray-500 mt-1">
            Conclua seus cursos para desbloquear seus certificados de especialização.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{CERTIFICATE_DATA.length - completedCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 p-6 bg-brand-dark/5 rounded-2xl border border-brand-dark/10 flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="text-center md:text-left">
            <h4 className="font-bold text-gray-900">Precisa de ajuda com seu certificado?</h4>
            <p className="text-sm text-gray-600">Se você concluiu todas as aulas e o certificado ainda não apareceu, entre em contato com nosso suporte.</p>
          </div>
...
const CertificateCard: React.FC<{ course: EnrolledCourse }> = ({ course }) => {
  ...
  return (
    <div className={`bg-white rounded-2xl border ${isCompleted ? 'border-brand-green/20' : 'border-gray-100'} shadow-sm overflow-hidden flex flex-col sm:flex-row transition-all hover:shadow-md`}>
      <div className="w-full sm:w-48 h-40 sm:h-auto relative flex-shrink-0">
        <img src={course.imageUrl} alt={course.title} className="w-full h-full object-cover" />
        <div className={`absolute inset-0 flex items-center justify-center bg-black/40 ${isCompleted ? 'opacity-100' : 'opacity-0'} transition-opacity`}>
          {isCompleted && <Award className="w-12 h-12 text-brand-accent drop-shadow-lg" />}
        </div>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-2">
          ...
          {isCompleted ? (
            <span className="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase">Disponível</span>
          ) : (
        ...
        <div className="mt-auto space-y-4">
          <div>
            <div className="flex justify-between text-xs font-bold mb-1.5">
              <span className={isCompleted ? 'text-brand-green' : 'text-gray-500'}>
                {isCompleted ? 'Concluído' : `${course.progress}% de Progresso`}
              </span>
              <span className="text-gray-400">{course.completedLessons}/{course.totalLessons} Aulas</span>
            </div>

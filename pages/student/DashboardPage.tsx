...
import { PlayCircle, Clock, Trophy, Flame, ArrowRight } from 'lucide-react';
...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!user?.uid) { setItems([]); setHoursWeek('0h 0m'); setCertCount(0); setStreakDays(0); return; }
    ...
    const enrollsMap = new Map<string, { ts?: Date; title?: string; imageUrl?: string; instructor?: string; category?: string }>();
    ...
    const recompute = () => {
      ...
      for (const [cid, e] of enrollsMap.entries()) {
        if (!coursesMap.has(cid)) {
          const lastAccessed = e.ts ? new Intl.RelativeTimeFormat('pt-PT', { numeric: 'auto' }).format(Math.round((e.ts.getTime() - Date.now())/3600000), 'hour') : undefined;
          list.push({
            id: cid,
            title: e.title || 'Curso',
            category: e.category || 'Geral',
            imageUrl: e.imageUrl || 'https://via.placeholder.com/320x180.png?text=Curso',
    ...
    const subscribeCoursesAndProgress = (courseIds: string[]) => {
      ...
      chunks.forEach(ids => {
        ...
        const us = onSnapshot(qS, (snap) => {
          ...
          // hours this week: 20 min per submission
          const minutes = subsCountWeek * 20;
          const hh = Math.floor(minutes/60); const mm = minutes % 60;
          setHoursWeek(`${hh}h ${mm}m`);
          // streak: consecutive days from today with activity
          const today = new Date(); today.setHours(0,0,0,0);
          let streak = 0;
          for (let i=0;i<60;i++) { // cap 60 days
            const d = new Date(today); d.setDate(today.getDate()-i);
            const key = d.toISOString().slice(0,10);
            if (i===0 || streak>0) {
              if (daysSet.has(key)) streak += 1; else if (i===0) { // no activity today -> streak 0
                break;
              } else { break; }
            }
          }
    ...
    const enrollHandler = (snap: any) => {
      ...
      snap.docs.forEach((d: any) => {
        ...
        const newTs = ts && (!prev.ts || ts.getTime() > prev.ts.getTime()) ? ts : prev.ts;
        enrollsMap.set(cid, {
          ts: newTs,
    ...
    enrollUnsubA = onSnapshot(query(collection(db, 'enrollments'), where('user_uid', '==', user.uid)), enrollHandler);
    enrollUnsubB = onSnapshot(query(collection(db, 'enrollments'), where('userId', '==', user.uid)), enrollHandler);
  ...
  return (
    <StudentLayout>
      <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Olá, {firstName}! 👋</h1>
          <p className="text-gray-500 mt-1 font-medium">Vamos continuar aprendendo hoje? Você tem ótimas atividades pendentes.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section>
              ...
              {lastCourse ? (
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col sm:flex-row gap-6 items-start">
                  ...
                  >
                    <img src={lastCourse.imageUrl} alt={lastCourse.title} className="w-full h-full object-cover" />
                  ...
                  <div className="flex-1 w-full">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-xs text-gray-400">{lastCourse.lastAccessed}</span>
                    </div>
                    <h3 className="text-xl font-bold text-gray-900 mb-1">{lastCourse.title}</h3>
                    <p className="text-sm text-gray-500 mb-4">Próxima aula: <span className="font-medium text-gray-700">{(lastCourse as any).nextLessonTitle || ''}</span></p>
                    
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs font-medium text-gray-500">
                        <span>{lastCourse.progress}% Concluído</span>
                        <span>{lastCourse.completedLessons}/{lastCourse.totalLessons} Aulas</span>
                      </div>
                ...
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm text-center text-sm text-gray-500">Sem cursos para continuar.</div>
              )}
            </section>
            ...
            <section>
              <div className="flex justify-between items-center mb-4">
                ...
                <Link to="/aluno/cursos" className="text-sm font-semibold text-brand-green hover:underline flex items-center gap-1">
                ...
                {items.slice(1).map(course => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <h4 className="font-bold text-gray-900 text-sm truncate mb-1 group-hover:text-brand-green transition-colors">{course.title}</h4>
                      ...
                      <span className="text-xs text-gray-500 font-medium">{course.progress}% Completo</span>
                    </div>
                  </Link>
                ))}
                
                {/* Discover More Card */}
                <Link to="/cursos" className="border-2 border-dashed border-gray-200 p-4 rounded-xl flex flex-col items-center justify-center text-center hover:border-brand-green hover:bg-green-50/50 transition-all group">
                  ...
                  <span className="text-sm font-bold text-gray-600 group-hover:text-brand-green">Descobrir novos cursos</span>
                </Link>
              </div>
            </section>
          ...
          <div className="space-y-8">
            <section>
              <h2 className="text-lg font-bold text-gray-900 mb-4">Recomendado para você</h2>
              <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm space-y-4">
                {recommended.map((course: any) => (
                  <Link to={`/cursos/${course.id}`} key={course.id} className="block group cursor-pointer">
                    <div className="h-24 rounded-lg overflow-hidden mb-2 relative">
                      <img src={course.imageUrl || course.image || 'https://via.placeholder.com/640x360.png?text=Curso'} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ...
                    <p className="text-xs text-gray-500">{course.instructor || 'Instrutor'}</p>
                  </Link>
                ))}
                {recommended.length === 0 && (
                  <div className="text-xs text-gray-500">Sem recomendações no momento.</div>
                )}
              </div>
            </section>
          </div>

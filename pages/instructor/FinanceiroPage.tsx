...
const FinanceiroPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!user?.uid) {
      setAvailable(0); setTotal(0); setPending(0); setTransactions([]); return;
    }
    ...
    const subscribeEnrollments = (courseDocs: any[]) => {
      ...
      if (courseIds.length === 0) { setAvailable(0); setTotal(0); setPending(0); setTransactions([]); return; }
      ...
      chunks.forEach(ids => {
        ...
        const handleSnap = async (snap: any) => {
          snap.docs.forEach((d: any) => { mapById.set(d.id, { id: d.id, ...d.data() }); });

          const items: Array<{ ts: Date; amount: number; course_id: string; title: string }>= [];
          Array.from(mapById.values()).forEach((rec: any) => {
            ...
            if (!cm || !cm.paid) return;
            items.push({ ts, amount: cm.price, course_id, title: cm.title });
          });
          ...
          const txs: Transaction[] = items.slice(0, 100).map((it, idx) => ({
            ...
            type: 'sale',
            description: `Venda: ${it.title}`,
            amount: it.amount,
            date: new Intl.DateTimeFormat('pt-PT', { dateStyle: 'medium', timeStyle: 'short' }).format(it.ts),
            status: 'completed',
          }));
          setTransactions(txs);
          ...
          const pend = items.filter(it => it.ts.getTime() >= sevenDaysAgo).reduce((acc, it) => acc + it.amount, 0);
          setTotal(sum);
          setPending(pend);
      ...
      if (snap.empty) {
        fallbackUnsub = onSnapshot(query(collection(db, 'courses'), where('creator_uid', '==', user.uid)), (snap2) => {
          if (snap2.empty) {
            ...
            if (name) {
              nameUnsub = onSnapshot(query(collection(db, 'courses'), where('instructor', '==', name)), (snap3) => subscribeEnrollments(snap3.docs));
            } else { subscribeEnrollments([]); }
          } else { subscribeEnrollments(snap2.docs); }
        });
      } else { subscribeEnrollments(snap.docs); }
    });
      ...
      <div className="max-w-7xl mx-auto space-y-8 pb-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <p className="text-slate-500 mt-1">Gerencie seus recebimentos, acompanhe seu saldo e solicite saques.</p>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50/50">
                      ...
                      <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor (MZM)</th>
                      ...
                      <tr key={t.id} className="hover:bg-slate-50/30 transition-colors group">
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg ${t.type === 'sale' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                        ...
                        <td className="px-6 py-5 text-xs font-medium text-slate-400">{t.date}</td>
                        <td className={`px-6 py-5 text-sm font-black ${t.amount > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                          {t.amount > 0 ? '+' : ''}{t.amount.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })}
                        </td>
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-2xl">
               ...
               <h3 className="text-lg font-bold mb-6">Próximo Pagamento Automático</h3>
               <div className="flex items-end gap-3 mb-8">
                 <span className="text-4xl font-black text-brand-accent">05</span>
                 <span className="text-lg font-bold text-slate-400 mb-1.5 uppercase tracking-widest">Novembro</span>
               </div>
               <div className="space-y-4">
                  ...
                  <p className="text-[10px] text-slate-400 leading-relaxed font-medium">
                    As transferências automáticas ocorrem todo dia 05 de cada mês para saldos acima de MZM 5.000,00.
                  </p>
               </div>
            </div>
            ...
            <div className="bg-brand-light/20 border border-brand-green/10 rounded-3xl p-8">
               ...
               <h4 className="font-bold text-brand-dark mb-2">Segurança nos Saques</h4>
               <p className="text-xs text-brand-dark/60 leading-relaxed mb-6 font-medium">
                 Para sua proteção, todas as solicitações de saque manual passam por uma análise de até 48 horas úteis.
               </p>
           ...
           <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="p-8 text-center space-y-6">
                 ...
                 <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Quanto você deseja retirar hoje?</p>
                 </div>
                 
                 <div className="space-y-4 text-left">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor do Saque (MZM)</label>
                       <div className="relative">
                          ...
                          />
                          <span className="absolute right-6 top-1/2 -translate-y-1/2 text-xs font-black text-slate-400">MZM</span>
                       </div>
                    </div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Saldo disponível: <span className="text-brand-green">MZM 8.450,20</span></p>
                 </div>
                 ...
                 <div className="flex gap-3">
                    ...
                    <button 
                      onClick={() => { alert('Solicitação enviada!'); setIsWithdrawModalOpen(false); }}
...
const FinanceCard = ({ label, value, icon, description, accent }: any) => (
  <div className={`bg-white p-6 rounded-3xl border shadow-sm flex flex-col gap-4 group transition-all hover:shadow-md ${accent ? 'border-brand-green/20' : 'border-slate-100'}`}>
    <div className="flex justify-between items-start">
      <div className={`p-3 rounded-2xl ${accent ? 'bg-brand-green/5' : 'bg-slate-50'} group-hover:scale-110 transition-transform`}>
        {icon}
      </div>
    ...
    <div>
      ...
      <h3 className={`text-2xl font-black ${accent ? 'text-brand-green' : 'text-slate-900'}`}>{value}</h3>

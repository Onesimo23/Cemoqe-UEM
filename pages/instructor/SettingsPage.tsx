...
const SettingsPage: React.FC = () => {
  ...
  const instructorAvatarUrl = previewUrl || profile?.avatar_url || user?.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(instructorName)}&background=0e7038&color=fff&size=120`;
  ...
  const handleChangeFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isValidType) { alert('Selecione uma imagem PNG ou JPG.'); return; }
    if (!isValidSize) { alert('Tamanho máximo permitido é 5MB.'); return; }
    try {
      setUploading(true);
      ...
      if (preferSupabase) {
        try {
          ...
          for (const b of buckets) {
            const { error: upErr } = await supabase.storage.from(b as any).upload(supaPath, file, { upsert: true, contentType: file.type });
            if (!upErr) {
              const { data: pub } = supabase.storage.from(b as any).getPublicUrl(supaPath);
      ...
      await setDoc(doc(db, 'profiles', user.uid), { avatar_url: finalUrl, updatedAt: serverTimestamp() }, { merge: true });
  ...
  return (
    <InstructorLayout>
      <div className="max-w-4xl mx-auto space-y-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Configurações do Painel</h1>
          <p className="text-slate-500 mt-1">Gerencie suas informações pessoais e preferências de recebimento.</p>
        </div>

        <div className="flex flex-col md:flex-row gap-8">
          {/* Side Menu */}
          <aside className="w-full md:w-64 space-y-2">
             <SettingsTab active={activeTab === 'profile'} icon={<User size={18} />} label="Perfil do Instrutor" onClick={() => setActiveTab('profile')} />
             <SettingsTab active={activeTab === 'payout'} icon={<CreditCard size={18} />} label="Dados de Pagamento" onClick={() => setActiveTab('payout')} />
             <SettingsTab active={activeTab === 'notifications'} icon={<Bell size={18} />} label="Notificações" onClick={() => setActiveTab('notifications')} />
          </aside>
          ...
          <div className="flex-1 bg-white rounded-3xl border border-gray-100 shadow-sm p-8 space-y-10">
               ...
               <div className="space-y-8 animate-in fade-in duration-300">
                  <div className="flex items-center gap-6">
                     <div className="relative group">
                        <img src={instructorAvatarUrl} className="w-24 h-24 rounded-3xl border-4 border-slate-50 shadow-md object-cover" alt={instructorName} />
                        {uploading && (
                          <div className="absolute inset-0 bg-black/30 rounded-3xl grid place-items-center text-white text-xs font-bold">Carregando...</div>
                        )}
                        <button onClick={handlePickImage} className="absolute bottom-0 right-0 p-2 bg-brand-green text-white rounded-xl shadow-lg hover:scale-110 transition-transform">
                          <Camera size={16} />
                        </button>
                        <input ref={fileInputRef} type="file" accept="image/png,image/jpeg" className="hidden" onChange={handleChangeFile} />
                     </div>
                     <div>
                        <h4 className="font-bold text-slate-900">{instructorName}</h4>
                        <p className="text-sm text-slate-400">Instrutora desde Jan 2024</p>
                        <span className="mt-2 inline-flex text-[10px] font-bold bg-brand-green text-white px-2 py-0.5 rounded uppercase">Verificada</span>
                     </div>
                  </div>
             ...
             {activeTab === 'payout' && (
               <div className="space-y-8 animate-in fade-in duration-300">
                  <div className="p-6 bg-brand-light/30 border border-brand-green/10 rounded-2xl flex gap-4">
                     <CreditCard size={24} className="text-brand-green flex-shrink-0" />
                     <div>
                        <h4 className="font-bold text-brand-dark text-sm">Transferências em MZM</h4>
                        <p className="text-xs text-brand-dark/60 mt-1 font-medium">Seus ganhos são transferidos mensalmente para sua conta bancária moçambicana cadastrada.</p>
                     </div>
                  </div>

                  <div className="space-y-6">
                     <div className="space-y-2">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Banco Destinatário</label>
                        <Select 
                          className="w-full" 
                          value={selectedBank}
                        ...
                        >
                          ...
                          <SelectPopover>
                            <SelectListBox>
                              <SelectItem value="BCI - Banco Comercial e de Investimentos">BCI - Banco Comercial e de Investimentos</SelectItem>
                              <SelectItem value="Standard Bank Moçambique">Standard Bank Moçambique</SelectItem>
                              <SelectItem value="Millennium bim">Millennium bim</SelectItem>
                              <SelectItem value="Absa Bank Moçambique">Absa Bank Moçambique</SelectItem>
                     ...
                     <div className="space-y-2">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest">NIB (Número de Identificação Bancária)</label>
                        <input 
                          type="text" 
             ...
             {activeTab === 'notifications' && (
               <div className="space-y-6 animate-in fade-in duration-300">
                  <ToggleOption label="Nova Matrícula" description="Receber e-mail sempre que um aluno se inscrever no curso." defaultChecked />
                  <ToggleOption label="Nova Dúvida" description="Avisar no Telegram sobre novas perguntas pendentes." defaultChecked />
                  <ToggleOption label="Relatório Semanal" description="Resumo de performance de todos os cursos por e-mail." />
               </div>
             )}
...
const SettingsTab = ({ active, icon, label, onClick }: any) => (
  <button 
    ...
    className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold transition-all ${
      active ? 'bg-brand-green text-white shadow-lg shadow-green-900/20' : 'text-slate-500 hover:bg-white hover:text-slate-900 border border-transparent hover:border-slate-100'
    }`}
  >
    {icon}
    {label}
...
const SelectListBox = ({ children }: any) => {
  return <div className="p-1 max-h-60 overflow-y-auto custom-scrollbar">{children}</div>;
};

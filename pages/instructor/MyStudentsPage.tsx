...
const MyStudentsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!user?.uid) { setStudents([]); return; }
    ...
    const subscribeEnrollments = (courseDocs: any[]) => {
      ...
      courseDocs.forEach((d) => { courseIds.push(d.id); courseMap[d.id] = (d.data() as any)?.title || 'Curso'; });
      if (courseIds.length === 0) { setStudents([]); return; }
      ...
      subsUnsub = onSnapshot(query(collection(db, 'submissions'), where('instructor_uid', '==', user.uid)), (s) => {
        ...
        setStudents(prev => prev.map(p => {
          ...
          if (entry && entry.data?.createdAt?.toDate) {
            return { ...p, lastActivity: new Intl.DateTimeFormat('pt-PT', { dateStyle: 'short', timeStyle: 'short' }).format(entry.data.createdAt.toDate()) };
          }
          return p;
        }));
      });
      ...
      chunks.forEach((ids) => {
        ...
        const handleSnap = async (enrSnap: any) => {
          ...
          for (const rec of records) { byUser[rec.user_uid] = rec; }

          const profiles = await Promise.all(Object.keys(byUser).map(async (uid) => {
            let name = 'Aluno'; let email = ''; let avatar = 'https://i.pravatar.cc/150?u=' + uid;
            try {
              const ps = await getDoc(doc(db, 'profiles', uid));
              if (ps.exists()) { const pd: any = ps.data(); name = pd?.full_name || name; email = pd?.email || ''; if (pd?.avatar_url) avatar = pd.avatar_url; }
            } catch {}
            const enr = byUser[uid];
            const course = courseMap[enr.course_id] || 'Curso';
            const lastDate = enr?.enrolledAt?.toDate ? enr.enrolledAt.toDate() : (enr?.createdAt?.toDate ? enr.createdAt.toDate() : null);
            const lastActivity = lastDate ? new Intl.DateTimeFormat('pt-PT', { dateStyle: 'short', timeStyle: 'short' }).format(lastDate) : '';
            const progress = 0;
            const engagement: Student['engagement'] = 'normal';
            const status: Student['status'] = 'active';
            return { id: uid, name, email, avatar, course, progress, lastActivity, engagement, status } as Student;
          }));

          // Atualiza imediatamente a lista com perfis obtidos (sem depender de submissões)
          setStudents(prev => {
            const map = new Map(prev.map(x => [x.id, x]));
            profiles.forEach(m => map.set(m.id, m));
    ...
    const subscribeEnrollmentsByInstructorNames = (names: string[]) => {
      ...
      if (!names || names.length === 0) { setStudents([]); return; }

      const qA = query(collection(db, 'enrollments'), where('instructor', 'in', names));
      const qB = query(collection(db, 'enrollments'), where('instructorName', 'in', names));

      const handleSnap = async (snap: any) => {
        const records = snap.docs.map((d: any) => {
          const raw: any = d.data();
    ...
    const primaryUnsub = onSnapshot(query(collection(db, 'courses'), where('instructor_uid', '==', user.uid)), (snap) => {
      if (snap.empty) {
        ...
        fallbackUnsub = onSnapshot(query(collection(db, 'courses'), where('creator_uid', '==', user.uid)), (snap2) => {
          if (snap2.empty) {
            ...
            const possibleNames = Array.from(new Set([ (user.displayName || '').trim() ].filter(Boolean)));
            if (fallbackNameUnsub) { fallbackNameUnsub(); fallbackNameUnsub = null; }
            if (possibleNames.length > 0) {
              fallbackNameUnsub = onSnapshot(query(collection(db, 'courses'), where('instructor', 'in', possibleNames)), (snap3) => {
                if (snap3.empty) {
              ...
              if (possibleNames.length > 0) subscribeEnrollmentsByInstructorNames(possibleNames);
  ...
  const filteredStudents = students.filter(s => {
    ...
    const matchesCourse = filterCourse === 'Todos os Cursos' || s.course.includes(filterCourse);
    const matchesStatus = filterStatus === 'Qualquer Status' || 
  ...
  const toggleStudentStatus = (id: string) => {
    ...
    if (selectedStudent?.id === id) {
      setSelectedStudent(prev => prev ? { ...prev, status: prev.status === 'active' ? 'blocked' : 'active' } : null);
    }
  };
    ...
    <InstructorLayout>
      <div className="max-w-7xl mx-auto space-y-8 pb-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Meus Alunos</h1>
            <p className="text-slate-500 mt-1">Gerencie inscritos, acompanhe o progresso e interaja diretamente.</p>
          </div>
          <div className="bg-white border border-slate-100 p-4 rounded-2xl flex items-center gap-4 shadow-sm">
            ...
            <div>
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Total Ativos</p>
               <p className="text-xl font-black text-slate-900 leading-none">{students.filter(s => s.status === 'active').length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              <SelectPopover>
                <SelectListBox>
                  <SelectItem value="Todos os Cursos">Todos os Cursos</SelectItem>
            ...
            >
              ...
              <SelectPopover>
                <SelectListBox>
                  <SelectItem value="Qualquer Status">Qualquer Status</SelectItem>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50/50 border-b border-slate-100">
                <tr>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Estudante</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Curso & Progresso</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Engajamento</th>
                  ...
                  <tr key={student.id} className="hover:bg-slate-50/30 transition-colors group">
                    <td className="px-6 py-6">
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <img src={student.avatar} className="w-11 h-11 rounded-xl border-2 border-white shadow-sm" alt={student.name} />
                          <div className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-white ${student.status === 'active' ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                        </div>
                        <div>
                          <p className="font-bold text-slate-900 group-hover:text-brand-green transition-colors">{student.name}</p>
                    ...
                    <td className="px-6 py-6">
                      <div className="max-w-[220px]">
                        <p className="text-xs font-black text-slate-700 truncate mb-2 uppercase tracking-tight">{student.course}</p>
                        <div className="flex items-center gap-3">
                           ...
                           <span className="text-[11px] font-black text-slate-900">{student.progress}%</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-6">
                       <div className="flex flex-col gap-1.5">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase w-fit border ${
                            student.engagement === 'dedicated' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                            student.engagement === 'at_risk' ? 'bg-red-50 text-red-600 border-red-100' :
                            'bg-blue-50 text-blue-600 border-blue-100'
                          }`}>
                            <Activity size={10} />
                            {student.engagement === 'dedicated' ? 'Dedicado' : 
                             student.engagement === 'at_risk' ? 'Em Risco' : 'Normal'}
                          </span>
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Visto: {student.lastActivity}</span>
                       </div>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button 
                          ...
                          className={`p-3 rounded-xl transition-all shadow-sm border ${
                            student.status === 'blocked' ? 'text-emerald-500 bg-emerald-50 border-emerald-100' : 'text-slate-400 bg-white hover:text-red-500 hover:bg-red-50 hover:border-red-100 border-transparent'
                          }`}
                          title={student.status === 'blocked' ? 'Desbloquear Aluno' : 'Bloquear Aluno'}
                        >
                          {student.status === 'blocked' ? <CheckCircle size={18} /> : <Ban size={18} />}
                        </button>
                      </div>
                    </td>
                  </tr>
          ...
          <div className="p-8 bg-slate-50/30 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
             <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Mostrando {filteredStudents.length} de {students.length} inscritos</p>
             <div className="flex gap-2">
                <button className="px-6 h-10 text-xs font-black uppercase text-slate-300 bg-white border border-slate-200 rounded-xl cursor-not-allowed">Anterior</button>
                <button className="px-6 h-10 text-xs font-black uppercase text-brand-green bg-white border border-brand-green/20 rounded-xl hover:bg-brand-green hover:text-white transition-all shadow-sm">Próximo</button>
             </div>
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-[100] flex justify-end bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
             <div className="w-full max-w-lg bg-white h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
                <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                   <div className="flex items-center gap-4">
                      <img src={selectedStudent.avatar} className="w-14 h-14 rounded-2xl border-2 border-white shadow-md" />
                      <div>
                        <h3 className="font-black text-slate-900 text-lg">{selectedStudent.name}</h3>
                        <p className="text-xs text-slate-500 font-bold uppercase tracking-tight">{selectedStudent.email}</p>
                      </div>
                   </div>
                   <button onClick={() => setSelectedStudent(null)} className="p-2.5 text-slate-400 hover:text-slate-900 bg-white rounded-xl border border-slate-200 shadow-sm transition-all hover:rotate-90">
                     <X size={20} />
                   </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8">
                   <div className="grid grid-cols-2 gap-4">
                      <div className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                         <div className="flex items-center gap-2 text-slate-400 mb-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">Tempo Acadêmico</span>
                         </div>
                         <p className="text-2xl font-black text-slate-900">14h 25m</p>
                      </div>
                      <div className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                         <div className="flex items-center gap-2 text-slate-400 mb-2">
                            <TrendingUp size={14} className="text-emerald-500" />
                            <span className="text-[10px] font-black uppercase tracking-widest">Fator Evolução</span>
                         </div>
                   ...
                   <div className="space-y-4">
                      <h4 className="font-black text-slate-900 uppercase text-xs tracking-widest border-l-4 border-brand-green pl-3">Linha do Tempo</h4>
                      <div className="space-y-6 relative before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                         <HistoryItem title="Concluiu: Tipografia Avançada" date="Hoje, 10:45" />
                         <HistoryItem title="Iniciou Módulo de UI Design" date="Ontem, 16:20" />
                         <HistoryItem title="Quiz: Avaliação Teórica (9.5)" date="22 Out, 14:00" highlight />
                         <HistoryItem title="Upload de Projeto: Branding" date="20 Out, 11:30" />
                      </div>
                   </div>
                </div>
                ...
                <div className="p-8 border-t border-slate-50 bg-slate-50/30">
                   <button 
                    onClick={() => { setActiveChat(selectedStudent); setSelectedStudent(null); }}
        ...
        {activeChat && (
          <div className="fixed bottom-0 right-0 md:right-8 z-[110] w-full md:w-[400px] bg-white border border-slate-200 shadow-[0_-20px_50px_rgba(0,0,0,0.1)] rounded-t-[32px] flex flex-col animate-in slide-in-from-bottom duration-500">
             <div className="p-5 bg-slate-900 text-white flex items-center justify-between rounded-t-[32px]">
                <div className="flex items-center gap-3">
                   <div className="relative">
                      <img src={activeChat.avatar} className="w-10 h-10 rounded-2xl border-2 border-slate-800" />
                      <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-500 border-2 border-slate-900 rounded-full"></div>
                   </div>
                   <div>
                      <p className="text-sm font-black leading-tight uppercase tracking-tight">{activeChat.name}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Aluno Verificado</p>
                   </div>
                </div>
                <button onClick={() => setActiveChat(null)} className="p-2 text-slate-400 hover:text-white transition-colors bg-white/5 rounded-xl">
                  <X size={20} />
                </button>
             </div>
             ...
             <div className="h-[380px] overflow-y-auto p-6 space-y-6 bg-slate-50/50 custom-scrollbar">
                <div className="flex justify-center mb-4">
                   <span className="text-[10px] font-black text-slate-400 uppercase bg-white border border-slate-200 px-4 py-1 rounded-full">Histórico da Aula</span>
                </div>
                <ChatMessage text="Professor, a aula de Auto-Layout está incrível! Mas tenho uma dúvida sobre as constraints." isMine={false} time="10:30" />
                <ChatMessage text="Oi! Que bom que gostou. Pode mandar sua dúvida, estou aqui para ajudar." isMine={true} time="10:35" />
                <ChatMessage text="Como faço para manter o padding fixo no mobile sem quebrar o componente?" isMine={false} time="10:36" />
             </div>
...
const SelectTrigger = ({ children }: any) => {
  ...
  return (
    ...
    >
      {children}
      <ChevronDown className={`h-4 w-4 text-slate-400 transition-transform duration-300 ${isOpen ? 'rotate-180 text-brand-green' : ''}`} />
    </button>
  );
};
...
const SelectValue = () => {
  ...
  return <span className={!value ? 'text-slate-400' : 'text-slate-900 truncate'}>{value || placeholder}</span>;
};
  ...
  return (
    <button
      ...
      className={`relative flex w-full cursor-default select-none items-center rounded-xl py-3.5 pl-4 pr-10 text-xs font-bold outline-none transition-all ${
        isSelected ? 'bg-brand-green text-white' : 'text-slate-600 hover:bg-slate-50'
      }`}
    >
...
const HistoryItem = ({ title, date, highlight }: any) => (
  <div className="relative pl-7">
    <div className={`absolute left-0 top-1.5 w-4 h-4 rounded-full border-2 border-white shadow-sm z-10 ${highlight ? 'bg-brand-accent' : 'bg-slate-200'}`}></div>
    <div className="flex flex-col">
       <span className={`text-xs font-black uppercase tracking-tight ${highlight ? 'text-brand-dark' : 'text-slate-700'}`}>{title}</span>
...
const ChatMessage = ({ text, isMine, time }: any) => (
  <div className={`flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
    <div className={`max-w-[85%] p-4 rounded-3xl text-sm font-bold leading-relaxed ${
      isMine ? 'bg-brand-green text-white rounded-tr-none shadow-lg shadow-green-900/10' : 'bg-white text-slate-700 border border-slate-100 shadow-sm rounded-tl-none'
    }`}>
      {text}
    </div>
    <span className="text-[9px] font-black text-slate-400 mt-2 px-2 uppercase tracking-widest">{time}</span>
  </div>
);

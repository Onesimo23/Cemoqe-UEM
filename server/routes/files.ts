...
const ALLOWED_HOSTS = ['supabase.co', 'firebasestorage.googleapis.com', 'storage.googleapis.com'];

// Helper: check allowed host
const isAllowedHost = (hostname: string) => ALLOWED_HOSTS.some((a) => hostname.includes(a));

// Convert buffer to PDF using soffice (LibreOffice headless)
const convertToPdfViaSoffice = (input: Buffer, inputExt: string): Promise<Buffer> =>
  new Promise((resolve, reject) => {
    const tmpDir = os.tmpdir();
    const timestamp = Date.now();
    const inputFile = path.join(tmpDir, `pdf_conv_${timestamp}_input${inputExt}`);
    const outputDir = tmpDir;
    
    fs.writeFileSync(inputFile, input);
    console.log(`[PDF Conversion] Input file: ${inputFile}, size: ${input.length}`);
    ...
    proc.on('close', (code) => {
      try {
        if (code !== 0) {
          console.error(`[PDF Conversion] soffice error (code ${code}):`, errOutput);
...
router.get('/proxy', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!isAllowedHost(host)) return res.status(403).json({ error: 'Host not allowed' });
    ...
    if (format === 'pdf' || officeExts.includes(ext)) {
      try {
        console.log(`[Proxy] Fetching file for conversion: ${url.substring(0, 100)}...`);
        const resp = await axios.get(url, { responseType: 'arraybuffer', timeout: 30000 });
        const inputBuf = Buffer.from(resp.data);
        console.log(`[Proxy] Downloaded ${inputBuf.length} bytes, converting to PDF...`);
        ...
        console.error('[Proxy] Conversion failed:', convErr?.message || convErr);
    ...
    res.status(500).json({ statusCode: 500, error: 'Error', message: err?.message || 'Failed to fetch file' });
  }
});
